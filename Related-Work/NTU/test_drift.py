...
def load_model(test_config): 
	...
	parser.add_argument("--nd", type=int, default = 16, help="number of frames in a 3D input cube") 	  
	...
	args.group = 2	
	args.nz = 12	 
	# args.nd = 16 # now read in terminal
	...
	model.load_state_dict(torch.load(test_config["model_file"](args.model_save_folder), map_location = args.device))
	model.eval()
	return model, args
...
def compute_score(model, args):
	...
	with torch.no_grad():
		for b_idx, (batch_data) in enumerate(test_loader): 
			# uncomment the below if args.nd == 24 for drift (because some videos have 47 frames and (47 - 2*24 + 1) = 0)
			# if args.n_seqs[b_idx] <= 0:
			# 	continue
