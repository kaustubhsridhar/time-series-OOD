...
# Note: The following results will be generated for just 1 run. In the paper, we ran these experiments 5 times (with different seeds) and reported the mean and standard deviation (std). For AUROC, where std is negligible (except for snowy), the results for 1 run are very close to the ones reported in paper. Similar results are expected for detection delay but we found that for rainy, it came 0.05 seconds higher (0.918) than reported in paper (0.86). We will rectify it in the final version.
...
## Step 7: Generate results for Drift OODs (Figure 9 (left))
      python check_OOD_drift.py --gpu $0/1/2/3$ --cuda --ckpt saved_models/drift.pt --n 20 --save_dir drift_log --printTNR 1 --transformation_list speed shuffle reverse periodic identity
