...
python check_OOD_carla.py --gpu 0 --cuda --ckpt saved_models/carla_model.pt --n 20 --out_folder_name out_rainy/out --save_dir carla_log/rainy --transformation_list speed shuffle reverse periodic identity
'''
...
parser.add_argument('--ckpt', default='saved_models/carla.pt', help="path load the trained network")
...
parser.add_argument('--save_dir', type=str, default='carla_log/rainy', help='directory for saving p-vaues and fisher values - carla_log/foggy, carla_log/snowy, carla_log/nigt, carla_log/replay, carla_log/rainy')
...
parser.add_argument('--printTNR', type=int, default=0)
...
def checkOOD(n = opt.n):  
    ...
    # CAL set CE Loss
    orig_train_dataset = CARLADataset(root_dir='data/CARLA_dataset/training', clip_len=opt.cl,  train=True, transforms_= transforms, img_size=opt.img_size, use_image=opt.use_image, use_of=opt.use_of, transformation_list=opt.transformation_list)
    ...
    print("Calculating CE loss for Calibration set  n times")
    ...
    in_test_dataset = CARLADataset('data/CARLA_dataset/testing', clip_len=opt.cl, train=False, transforms_= transforms, img_size=opt.img_size, in_dist_test=True, use_image=opt.use_image, use_of=opt.use_of, transformation_list=opt.transformation_list)
    ...
    # Out-Dist CE loss
    out_test_dataset = CARLADataset('data/CARLA_dataset/testing', clip_len=opt.cl, train=False, transforms_= transforms, img_size=opt.img_size, in_dist_test=False, use_image=opt.use_image, use_of=opt.use_of, transformation_list=opt.transformation_list, out_folder_name=opt.out_folder_name)
...
if __name__ == "__main__":
    ...
    for trial in range(opt.trials):
        ...
        for i in range(opt.n):
            print("Calculating results for n: {} from the pre-computed fisher-values".format(n+1))
    ...
    print("Avg. Det Delay: ", (np.mean(det_delay_all_trials,0)[0]+1)*0.8) # detection delay in seconds - 16/20 is the time duration for 1 window
