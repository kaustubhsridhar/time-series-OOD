...
def animateGraph():
    ...
    def animate(i):
        ...
        try:
            ...
            ax2.clear()
            ax2.set_ylim(-5,30)
            ax2.set_yticks([-5,0,20,30])
            ax2.plot(sx,sy,color='red')
            ax2.set_title("Illumination level")
            ax2.set_ylabel("log(M)")
            ax2.set_xlabel("Time(s)")
            ax2.autoscale(enable=False, axis='y')
            ax5.clear()
            ax5.set_ylim(-5,30)
            ax5.set_yticks([-5,0,20,30])
            ax5.plot(a,m,color='green')
            ax5.set_title("Rain Intensity")
            ax5.set_ylabel("log(M)")
